...
OUTPUT_EBAY_REVISE_CSV = r"store_discovery\EBAY_BULK_REVISE_LISTINGS.csv"
...
def generate_ebay_revise_csv(part_numbers: list):
    ...
    print(f"\n=== GENERATING EBAY REVISE CSV (WITH FULL FITMENT TABLES) ===")
    ...
    for part_no in part_numbers:
        ...
        if db_match:
            ...
            # Key difference for Revision: Action = Revise, and CustomLabel connects to existing active SKU
            row_dict = {
    ...
    if ebay_rows:
        ...
        with open(OUTPUT_EBAY_REVISE_CSV, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
            writer.writeheader()
            writer.writerows(ebay_rows)

        print(f"\n=== SUCCESS! Generated eBay Revise CSV ===")
        print(f"Total Listings Revised: {len(ebay_rows)}")
        print(f"Output CSV File:       {OUTPUT_EBAY_REVISE_CSV}")

if __name__ == "__main__":
    if len(sys.argv) > 1:
        parts = sys.argv[1:]
        generate_ebay_revise_csv(parts)
    else:
        generate_ebay_revise_csv(["42064", "42065", "42067"])
